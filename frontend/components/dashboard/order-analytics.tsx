    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center soya-fade-in">
          ...
          <p className="text-gray-600 text-lg font-medium">{t('loadingAnalytics')}</p>
          <p className="text-gray-400 text-sm mt-1">{t('crunchingNumbers')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!analyticsData) {
    return (
      <div className="text-center py-12 soya-fade-in">
        ...
        <p className="font-semibold text-gray-900">{t('failedToLoad')}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 soya-fade-in">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">{overview.orders_per_day.toFixed(1)} {t('perDayAvg')}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">{overview.avg_order_size_tm.toFixed(1)} tm {t('avgSize')}</p>
              </div>
        ...
        <Card className="soya-card border-0 overflow-hidden">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">{t('fulfillmentRate')}</p>
                <p className="text-3xl font-bold text-gray-900 mt-1">{overview.fulfillment_rate.toFixed(1)}%</p>
                <p className="text-xs text-gray-400 mt-1">{t('orderCompletionRate')}</p>
              </div>
        ...
        <Card className="soya-card border-0 overflow-hidden">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">{t('activeClients')}</p>
                <p className="text-3xl font-bold text-gray-900 mt-1">{overview.unique_clients.toLocaleString()}</p>
                <p className="text-xs text-gray-400 mt-1">{client_segmentation.repeat_rate.toFixed(1)}% {t('repeatRate')}</p>
              </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3 soya-fade-in soya-stagger-1">
        <Card className="soya-card border-0 overflow-hidden">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-gray-500">{t('volumeGrowth')}</p>
            ...
            <p className="text-xs text-gray-400 mt-1">{t('periodOverPeriod')}</p>
          </CardContent>
        </Card>
        ...
        <Card className="soya-card border-0 overflow-hidden">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-gray-500">{t('onTimeDelivery')}</p>
            ...
            <p className="text-xs text-gray-400 mt-1">
              {delivery_performance.on_time_count} {t('ofTotal')} {delivery_performance.total_delivered} {t('delivered')}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="soya-card border-0 overflow-hidden">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-gray-500">{t('avgDeliveryTime')}</p>
              <div className="p-1.5 bg-orange-100 rounded-lg">
                <Calendar className="h-4 w-4 text-orange-600" />
              </div>
            </div>
            <p className="text-3xl font-bold text-gray-900">{delivery_performance.avg_delivery_days.toFixed(1)} {t('days')}</p>
            <p className="text-xs text-gray-400 mt-1">
              {t('range')}: {delivery_performance.min_delivery_days}-{delivery_performance.max_delivery_days} {t('days')}
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="pt-6">
            ...
            <TabsContent value="trends" className="space-y-6 mt-0">
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('monthlyOrderTrends')}</h3>
                <p className="text-sm text-gray-500 mb-4">{t('orderVolumeOverTime')}</p>
                <ResponsiveContainer width="100%" height={350}>
                  <LineChart data={analyticsData.monthly_trends}>
                    ...
                    />
                    <Legend />
                    <Line yAxisId="left" type="monotone" dataKey="total_volume" stroke={chartColors.primary} strokeWidth={2} name={t('volume')} dot={{ fill: chartColors.primary, strokeWidth: 2 }} />
                    <Line yAxisId="right" type="monotone" dataKey="order_count" stroke={chartColors.secondary} strokeWidth={2} name={t('orderCount')} dot={{ fill: chartColors.secondary, strokeWidth: 2 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <div className="bg-white rounded-xl border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('orderSizeDistribution')}</h3>
                  <p className="text-sm text-gray-500 mb-4">{t('ordersBySize')}</p>
                  <div className="space-y-5">
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-700">{t('small')} ({analyticsData.order_size_distribution.small_orders.description})</span>
                      ...
                      <p className="text-xs text-gray-400 mt-1">{analyticsData.order_size_distribution.small_orders.count.toLocaleString()} {t('orders')}</p>
                    </div>
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-700">{t('medium')} ({analyticsData.order_size_distribution.medium_orders.description})</span>
                      ...
                      <p className="text-xs text-gray-400 mt-1">{analyticsData.order_size_distribution.medium_orders.count.toLocaleString()} {t('orders')}</p>
                    </div>
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-700">{t('large')} ({analyticsData.order_size_distribution.large_orders.description})</span>
                      ...
                      <p className="text-xs text-gray-400 mt-1">{analyticsData.order_size_distribution.large_orders.count.toLocaleString()} {t('orders')}</p>
                    </div>
                  </div>
                </div>

                <div className="bg-white rounded-xl border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('recentActivity')}</h3>
                  <p className="text-sm text-gray-500 mb-4">{t('last7and30Days')}</p>
                  <div className="space-y-4">
                    <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-semibold text-green-900">{t('last7Days')}</span>
                        <div className="p-1.5 bg-green-200 rounded-lg">
                          <Zap className="h-4 w-4 text-green-700" />
                        </div>
                      </div>
                      <p className="text-3xl font-bold text-green-700">
                        {analyticsData.recent_activity.last_7_days.order_count} {t('orders')}
                      </p>
                      <p className="text-sm text-green-600 mt-1">
                        {analyticsData.recent_activity.last_7_days.total_volume.toFixed(1)} {t('tmDelivered')}
                      </p>
                    </div>
                    <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 border border-yellow-200 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-semibold text-yellow-900">{t('last30Days')}</span>
                        <div className="p-1.5 bg-yellow-200 rounded-lg">
                          <Package className="h-4 w-4 text-yellow-700" />
                        </div>
                      </div>
                      <p className="text-3xl font-bold text-yellow-700">
                        {analyticsData.recent_activity.last_30_days.order_count} {t('orders')}
                      </p>
                      <p className="text-sm text-yellow-600 mt-1">
                        {analyticsData.recent_activity.last_30_days.total_volume.toFixed(1)} {t('tmDelivered')}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <TabsContent value="clients" className="space-y-6 mt-0">
              <div className="grid gap-5 md:grid-cols-2">
                <div className="bg-white rounded-xl border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('topClientsByVolume')}</h3>
                  <p className="text-sm text-gray-500 mb-4">{t('highestVolumeDelivered')}</p>
                    ...
                    {client_segmentation.top_by_volume.slice(0, 5).map((client, index) => (
                      <div key={client.client_id} className="flex items-center justify-between p-3 bg-gradient-to-r from-green-50 to-white rounded-xl border border-green-100 hover:shadow-md transition-shadow">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-500">{client.order_count} {t('orders')}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('topClientsByFrequency')}</h3>
                  <p className="text-sm text-gray-500 mb-4">{t('mostFrequentOrders')}</p>
                  <div className="space-y-3">
                    {client_segmentation.top_by_frequency.slice(0, 5).map((client, index) => (
                      <div key={client.client_id} className="flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-white rounded-xl border border-yellow-100 hover:shadow-md transition-shadow">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-yellow-700">{client.order_count} {t('orders')}</p>
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('clientSegmentation')}</h3>
                <p className="text-sm text-gray-500 mb-4">{t('oneTimeVsRepeat')}</p>
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5 border border-green-200">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm font-medium text-green-700">{t('repeatClients')}</p>
                      <TrendingUp className="h-4 w-4 text-green-600" />
                    </div>
                    <p className="text-4xl font-bold text-green-700">{client_segmentation.repeat_clients.toLocaleString()}</p>
                    <p className="text-xs text-green-600 mt-1">{client_segmentation.repeat_rate.toFixed(1)}% {t('ofTotal')}</p>
                  </div>
                  <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl p-5 border border-yellow-200">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm font-medium text-yellow-700">{t('oneTimeClients')}</p>
                      <Users className="h-4 w-4 text-yellow-600" />
                    </div>
                    <p className="text-4xl font-bold text-yellow-700">{client_segmentation.one_time_clients.toLocaleString()}</p>
                    <p className="text-xs text-yellow-600 mt-1">{((client_segmentation.one_time_clients / client_segmentation.total_clients) * 100).toFixed(1)}% {t('ofTotal')}</p>
                  </div>
                </div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="products" className="mt-0">
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('productPerformance')}</h3>
                <p className="text-sm text-gray-500 mb-4">{t('salesByProduct')}</p>
                <div className="space-y-4">
                  {analyticsData.product_performance.map((product, index) => (
                    <div key={index} className="bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Badge className="soya-badge-success">{product.market_share.toFixed(1)}% {t('productMarketShare')}</Badge>
                      </div>
                      <div className="grid grid-cols-4 gap-4">
                        <div className="bg-green-50 rounded-lg p-3 text-center">
                          <p className="text-xs font-medium text-green-600 uppercase tracking-wider">{t('orders')}</p>
                          <p className="text-2xl font-bold text-green-700 mt-1">{product.order_count.toLocaleString()}</p>
                        </div>
                        <div className="bg-yellow-50 rounded-lg p-3 text-center">
                          <p className="text-xs font-medium text-yellow-600 uppercase tracking-wider">{t('volume')}</p>
                          <p className="text-2xl font-bold text-yellow-700 mt-1">{product.total_volume.toFixed(1)} tm</p>
                        </div>
                        <div className="bg-blue-50 rounded-lg p-3 text-center">
                          <p className="text-xs font-medium text-blue-600 uppercase tracking-wider">{t('avgSizeLabel')}</p>
                          <p className="text-2xl font-bold text-blue-700 mt-1">{product.avg_order_size.toFixed(1)} tm</p>
                        </div>
                        <div className="bg-purple-50 rounded-lg p-3 text-center">
                          <p className="text-xs font-medium text-purple-600 uppercase tracking-wider">{t('clients')}</p>
            ...
            <TabsContent value="geography" className="space-y-6 mt-0">
              <div className="grid gap-5 md:grid-cols-2">
                <div className="bg-white rounded-xl border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('ordersByCountry')}</h3>
                  <p className="text-sm text-gray-500 mb-4">{t('distributionAcrossCountries')}</p>
                  <div className="space-y-3">
                    {analyticsData.geographical_distribution.by_country.map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-100 hover:shadow-md transition-shadow">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{item.unique_clients} {t('clientsLabel')}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-green-700">{item.total_volume.toFixed(1)} tm</p>
                          <p className="text-xs text-gray-500">{item.order_count.toLocaleString()} {t('orders')}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-5">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('topCities')}</h3>
                  <p className="text-sm text-gray-500 mb-4">{t('highestVolumeCities')}</p>
                    ...
                    {analyticsData.geographical_distribution.by_city.slice(0, 10).map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-white rounded-xl border border-yellow-100 hover:shadow-md transition-shadow">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{item.country} • {item.unique_clients} {t('clientsLabel')}</p>
                        </div>
                        <div className="text-right">
            ...
            <TabsContent value="seasonal" className="mt-0">
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('seasonalOrderPatterns')}</h3>
                <p className="text-sm text-gray-500 mb-4">{t('orderActivityByMonth')}</p>
                <ResponsiveContainer width="100%" height={350}>
                  <BarChart data={analyticsData.seasonal_patterns}>
                    ...
                    <Legend />
                    <Bar yAxisId="left" dataKey="total_volume" fill={chartColors.primary} name={t('volume')} radius={[4, 4, 0, 0]} />
                    <Bar yAxisId="right" dataKey="order_count" fill={chartColors.secondary} name={t('orderCount')} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </TabsContent>
